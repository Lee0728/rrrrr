...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	...
	switch (section)
	{
		case SECTION_SERVER: // server settings
			return (_display_server_settings ? 6 : 0);
...
//            [[textCell textfield] setPlaceholder:NSLocalizedString(@"not set", @"not set placeholder")];
        ...
        case 3:
            ...
            [[textCell textfield] setSecureTextEntry:YES];
            //[[textCell textfield] setPlaceholder:NSLocalizedString(@"not set", @"not set placeholder")];
			break;
		case 4:
...
{	
	switch(indexPath.row) 
	{
		case 0:
        {
            ...
            NSString *logmodestring = [[NSString alloc] init];
            int logmode = [_params intForKey:@"logmode"];
            if(logmode==0)
            {
                logmodestring = @"用户名密码";
            }
            ...
            {
                logmodestring = @"windows账户";
            }
            [[selCell selection] setText:[NSString stringWithFormat:@"%@@%@@%d", logmodestring,resolution, colorBits]];
            break;
        }            
		case 1:
        {
...
- (void)handleSave:(id)sender
{
    ...
    if (![_bookmark isKindOfClass:NSClassFromString(@"TSXConnectComputerBookmark")])
    {
        ...
        {
            BlockAlertView* alertView = [BlockAlertView alertWithTitle:NSLocalizedString(@"Cancel without saving?", @"Incomplete bookmark error title") message:@"请输入服务器地址，及用户名密码信息"];
