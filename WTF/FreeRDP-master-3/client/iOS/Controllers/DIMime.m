...
unsigned int MIME_DECODE_TABLE[256] =
{
...
/****************************
 Encoding Core
 ***************************/
void MimeEncodeFullLines(const char *inputBuf,const unsigned int inputLen,const char *outputBuf)
{
    unsigned int B;
    unsigned int innerLimit,outerLimit;
    PByte3 inPtr;
    PByte4 outPtr;
...
void MimeEncodeNoCRLF(const char *inputBuf,const unsigned int inputLen,const char *outputBuf)
{
    unsigned B,innerLimit,outerLimit;
    PByte3 inPtr;
    PByte4 outPtr;
...
/****************************
 Decoding Core
 ****************************/
unsigned int MimeDecodePartial(const char *inputBuf,unsigned int inputBufLen,char *outputBuf,unsigned int *byteBuf,unsigned int *byteBufSpace)
{
    unsigned int lByteBuf,lBytebufSpace,C;
    unsigned char *inPtr,*outerLimit;
    PByte3 outPtr;
    
    if (inputBufLen > 0){
        inPtr = (unsigned char *)inputBuf;
        ...
        while (inPtr != outerLimit)
        {
            ...
            (*outPtr).b3 = (unsigned char)lByteBuf;
            lByteBuf >>= 8;
            
            (*outPtr).b2 = (unsigned char)lByteBuf;
            lByteBuf >>= 8;
            
            (*outPtr).b1 = (unsigned char)lByteBuf;
...
unsigned int MimeDecodepartialEnd(char *outputBuf,unsigned int byteBuf,unsigned int byteBufSpace,unsigned int endIdx)
{
    unsigned int lbyteBuf;
    char s[2];
    switch (byteBufSpace)
    {
    case 1:
            lbyteBuf = byteBuf >> 2;
            s[1] = (unsigned char)lbyteBuf;
            lbyteBuf >>= 8;
            s[0] = (unsigned char)lbyteBuf;
