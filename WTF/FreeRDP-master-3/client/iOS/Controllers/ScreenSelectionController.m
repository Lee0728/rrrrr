...
- (id)initWithConnectionParams:(ConnectionParams*)params keyPath:(NSString*)keyPath
{
    ...
    if (self)
    {
        ...
        printf("the _selection_resolution is %d",_selection_resolution);
        idx = [_params intForKeyPath:[self keyPathForKey:@"logmode"]];
        printf("the idx logmode is %d",idx);
        
        _selection_logmode = (idx != NSNotFound) ? idx : 0;
    }
    return self;    
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch ([indexPath section])
    {
        case 0:
            if(indexPath.row == 0)
            {
                ...
                [[cell textLabel] setText:@"用户名密码认证"];
                break;
            }
            else
            {
                cell = [self tableViewCellFromIdentifier:TableCellIdentifierMultiChoice];
                [[cell textLabel] setText:@"windows账户认证"];
                break;
            }
        case 1:
            cell = [self tableViewCellFromIdentifier:TableCellIdentifierMultiChoice];
    ...
    {
        printf("the indexpath row is %d",[indexPath row]);
        printf("the selection_logmode is %d",_selection_logmode);
        if([indexPath row]==_selection_logmode)
        {
    ...
    {
        if([indexPath row]==_selection_color)
        {
            [cell setAccessoryType:UITableViewCellAccessoryCheckmark];
        }
        else
    ...
    {
        if([indexPath row]==_selection_resolution)
        {
            [cell setAccessoryType:UITableViewCellAccessoryCheckmark];
        }
        else
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([indexPath section]==0)
    {
        cur_selection=_selection_logmode;
    }
    else if([indexPath section]==1)
    {
        cur_selection = _selection_color;
    }
    else
    {
        cur_selection=_selection_resolution;
    }
    printf("the cur_selection is %d",cur_selection);
	...
	if([indexPath row] != cur_selection)
	{
        ...
        if ([indexPath section] == 0)
        {
            int sel_value = [indexPath row];//[[_color_options valueForKey:[_color_options keyAtIndex:[indexPath row]]] intValue];
            
            // update selection index and params value
            [_params setInt:sel_value forKeyPath:[self keyPathForKey:@"logmode"]];
            _selection_logmode = [indexPath row];
        }
        else if ([indexPath section] == 1)
        {
        ...
        else
        {
            ...
            NSArray* indexPaths = [NSArray arrayWithObjects:[NSIndexPath indexPathForRow:[_resolution_modes count] inSection:2],
                                                            [NSIndexPath indexPathForRow:([_resolution_modes count] + 1) inSection:2], nil];
