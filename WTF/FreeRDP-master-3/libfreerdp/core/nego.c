...
BOOL nego_tcp_connect(rdpNego* nego)
{
	if (!nego->tcp_connected)
    {
        printf("the safemode is %d",nego->safegateway);
		nego->tcp_connected = transport_connect(nego->transport, nego->hostname, nego->port, nego->safegateway, nego->Sessionid);
    }
...
void nego_set_target(rdpNego* nego, char* hostname, int port,int safegateway,char* Sessionid)
{
	nego->hostname = hostname;
	nego->port = port;
    nego->safegateway = safegateway;
