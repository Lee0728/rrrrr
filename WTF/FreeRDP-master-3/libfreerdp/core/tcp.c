...
BOOL tcp_connect(rdpTcp* tcp, const char* hostname, UINT16 port, int safemode ,char* Sessionid)
{
	UINT32 option_value;
	socklen_t option_len;
    printf("the safemode is %d",safemode);
    printf("the Sessionid is %s",Sessionid);
	if (hostname == NULL)
		return FALSE;
	...
	else
	{
        ...
        //const char *Sessionid = "sid00000014";
        if(safemode == 1)
        {
        byte *packet = malloc(18);
        //char name[5]="AGS:";
        //char *str = strcat(name,Sessionid);
        //printf("the str is %s",str);
        //packet = (unsigned char*) (strcat("AGS:",Sessionid));
         packet = (unsigned char*)Sessionid;
        //packet = (unsigned char*)("AGS:sid00000214");
        tcp_write(tcp,packet,16);
       }
	}
