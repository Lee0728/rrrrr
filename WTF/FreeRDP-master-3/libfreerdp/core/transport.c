...
BOOL transport_connect(rdpTransport* transport, const char* hostname, UINT16 port, int safemode,char* Sessionid)
{
	BOOL status = FALSE;
	...
	else
	{
		...
		status = tcp_connect(transport->TcpIn, hostname, port,safemode,Sessionid);
